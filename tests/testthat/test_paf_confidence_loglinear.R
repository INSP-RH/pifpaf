context("CI of PAF using loglinear method")

test_that("Checking paf.confidence.loglinear",{
  
  #Expect error  when confidence levels are incorrectly specified
  expect_error({
    set.seed(347618)
    X        <- rnorm(100,3,1)
    thetahat <- 1.4
    thetavar <- 0.05
    rr       <- function(X,theta){exp(X*theta)}
    paf.confidence.loglinear(X = X, thetahat = thetahat, thetavar = thetavar, rr = rr, confidence = -1)
  })
  
  #Expect error  when  variance is not positive semi-definite
  
  expect_error({
    set.seed(347618)
    X        <- rnorm(100,3,1)
    thetahat <- 1.4
    thetavar <- -0.05
    rr       <- function(X,theta){exp(X*theta)}
    paf.confidence.loglinear(X = X, thetahat = thetahat, thetavar = thetavar, rr = rr, confidence = 95)
  })
  
  expect_error({
    set.seed(347618)
    X        <- rnorm(100,3,1)
    thetahat <- c(1.4, 1)
    thetavar <- matrix(c(.2,.9,.9, .1), ncol = 2, byrow = TRUE)
    rr       <- function(X,theta){X*theta[1]+theta[2]}
    paf.confidence.loglinear(X = X, thetahat = thetahat, thetavar = thetavar, rr = rr, confidence = 95)
  })
  
  #Check dimensions are correctly specified
  expect_error({
    set.seed(347618)
    X        <- rnorm(100,3,1)
    thetahat <- .02
    thetavar <- matrix(c(.2,.01,.01, .1), ncol = 2, byrow = TRUE)
    rr       <- function(X,theta){X*theta[1]+theta[2]}
    paf.confidence.loglinear(X = X, thetahat = thetahat, thetavar = thetavar, rr = rr, confidence = 95)
  })
  
  #All exposure levels are zero expect paf equal to zero
  expect_equal({
    set.seed(347618)
    X        <- rep(0,100)
    thetahat <- 0.2
    thetavar <- 0.02
    rr       <- function(X,theta){exp(X*theta)}
    paf.confidence.loglinear(X = X, thetahat = thetahat, thetavar = thetavar, rr = rr, confidence = 95)
  }, c("Lower" = 0, "Point_Estimate" =  0, "Upper" = 0))
  
  
})

test_that("Checking paf_confidence_approximate_loglinear",{
  
  #Check that if variance of X is zero and X is zero paf is zero and so the upper and lower bounds of its CI
  expect_equal({
    Xmean    <- 0
    Xvar     <- 0
    thetahat <- 0.2
    thetavar <- 0.003
    rr       <- function(X,theta){exp(X*theta)}
    paf.confidence.loglinear(X = Xmean, Xvar = Xvar, thetahat = thetahat, 
                             thetavar = thetavar, rr = rr, method = "approximate")
  },
  c("Lower" = 0, "Point_Estimate" =  0, "Upper" = 0) 
  )
  
  #Check that if there's no variance in theta the result is the same
  expect_equal({
    Xmean    <- 1
    Xvar     <- .3
    rr       <- function(X,theta){exp(X*theta)}
    theta    <- 0.2
    thetavar <- 0
    paf.confidence.loglinear(X = Xmean, Xvar = Xvar, thetahat = thetahat, 
                             thetavar = thetavar, rr = rr, method = "approximate")
  },{
    Xmean    <- 1
    Xvar     <- .3
    rr       <- function(X,theta){exp(X*theta)}
    theta    <- 0.2
    thetavar <- 0
    paf.confidence.loglinear(X = Xmean, Xvar = Xvar, thetahat = thetahat, 
                             thetavar = thetavar, rr = rr, method = "approximate")
  })
  
  #Check errors 
  #Covariance matrix of theta not positive semi-definite
  expect_error({
    Xmean   <- 1
    Xvar    <- .3
    rr      <- function(X,theta){exp(X*theta)}
    theta   <- 0.2
    thetavar <- - 0.02
    paf.confidence.loglinear(X = Xmean, Xvar = Xvar, thetahat = thetahat, 
                             thetavar = thetavar, rr = rr, method = "approximate")
  })
  
  #Exposure mean less than zero
  expect_warning({
    Xmean   <- -1
    Xvar    <- .3
    rr      <- function(X,theta){exp(X*theta)}
    theta   <- 0.2
    thetavar <- 0.02
    paf.confidence.loglinear(X = Xmean, Xvar = Xvar, thetahat = thetahat, 
                             thetavar = thetavar, rr = rr, method = "approximate")
  })
  
  #Covariance matrix of exposure values not positive semi-definite
  expect_error({
    Xmean   <- 1
    Xvar    <- - .3
    rr      <- function(X,theta){exp(X*theta)}
    theta   <- 0.2
    thetavar <- 0.02
    paf.confidence.loglinear(X = Xmean, Xvar = Xvar, thetahat = thetahat, 
                             thetavar = thetavar, rr = rr, method = "approximate")
  })
  
  #Exposure diminishes risk
  expect_warning({
    Xmean   <- 1
    Xvar    <- .3
    rr      <- function(X,theta){exp(X*theta)}
    theta   <- -0.2
    thetavar <- 0.02
    paf.confidence.loglinear(X = Xmean, Xvar = Xvar, thetahat = thetahat, 
                             thetavar = thetavar, rr = rr, method = "approximate")
  })
  
  
})
