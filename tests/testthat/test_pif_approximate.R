...
test_that("Checking pif.approximate convergence",{
  ...
  expect_equal(
    pif.approximate(X = 3, Xvar = 1, thetahat = 1, rr = function(X, theta){exp(theta*X)}),
    1 - 1/(1.5*exp(3))
  )
  
  #Check that approximate PIF works when counterfactual is identity
  expect_equal(
    pif.approximate(X = 3, Xvar = 1, thetahat = 1, rr = function(X, theta){exp(theta*X)}, cft = function(X){X}),
    0
  )
  
  #Check that approximate PAF works when RR is constant 1
  expect_equal(
    pif.approximate(X = 3, Xvar = 1, thetahat = 1, rr = function(X, theta){1}),
    0
  )
  
  #Check that approximate PIF works when RR is constant 1
  expect_equal(
    pif.approximate(X = 3, Xvar = 1, thetahat = 1, rr = function(X, theta){1}, cft = function(X){(0.4*X)^2}),
    0
  )
  
  #Check that approximate PIF works when counterfactual is constant 0 is equal to empiric
  expect_equal(
    pif.approximate(X = 3, Xvar = 1, thetahat = 1, rr = function(X, theta){1}, cft = function(X){0}),
    pif.approximate(X = 3, Xvar = 1, thetahat = 1, rr = function(X, theta){1})
  )
