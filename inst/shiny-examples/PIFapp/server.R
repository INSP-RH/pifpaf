...
shinyServer(function(input,output){
                              ...
                              if(is.null(input$weight)){
                                ...
                                if(file_ext(input$weight$name)!="csv"){
                                  ...
                                  if(length(Weight)!=length(X)){
                                    ...
                                    PAF.interval <- paf.confidence(X=X, thetahat = theta, thetavar = SD, rr = RR, weights = Weight)
                                    return(sprintf("Confidence interval PAF :  (%f, %f) <br/> Point Estimate :  %f ", PAF.interval[1], PAF.interval[3], PAF.interval[2]))
                                  }
                                }
                              }
  ...
  output$D3plot <- renderPlot({
    rr    <- as.numeric(input$rr)
    mina  <- as.numeric(input$mina)
    maxa  <- as.numeric(input$maxa)
    minb  <- as.numeric(input$minb)
    maxb  <- as.numeric(input$maxb)
    
    if(mina >= maxa || minb >= maxb){
      Error <- 23; return(Warnings2(Error));
    }else{
          ...
          if (input$xmin0 ==1) {
            if (rr <5){
              ...
              if(theta<0){
                ...
                if (input$w==0) {
                  ...
                  PAF.Plot <- pif.counterfactual.3D(X=X, thetahat = theta, rr = RR, mina = mina, maxa = maxa, minb = minb, maxb = maxb )
                  return(PAF.Plot)
                }
                  ...
                  if(is.null(input$weight)){
                    ...
                    if(file_ext(input$weight$name)!="csv"){
                      ...
                      if(length(Weight)!=length(X)){
                        ...
                        PAF.Plot <- pif.counterfactual.3D(X=X, thetahat = theta, rr = RR, weights = Weight, mina = mina, maxa = maxa, minb = minb, maxb = maxb )
                        return(PAF.Plot)
            ...
            if(xmin<0){
              ...
              if(rr < 5){
                ...
                Error <- 24; return(Warnings2(Error));
              }
            }
          }
        }
