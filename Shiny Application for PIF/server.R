      ...
      if(file_ext(input$X$name)!="csv"){
        ...
        if (input$xmin0 ==1) {
          if (rr <5){
              ...
              if (input$w==0) {
                ...
                if(input$PIForPAF > 1){
                  if(input$Cft == 1){
                    ...
                    if(is.null(input$Cftfun)){
                      ...
                      if(file_ext(input$Cftfun$name)!="R"){
                        return(ggplot()+ggtitle("Warning: Incorrect file extension. Counterfactual function must be written in an R file"))
                      }else{
                        source(input$Cftfun$datapath)
                        if(exists("Counterfactual") == FALSE){
                          return(ggplot()+ggtitle("Warning: There is no function named Counterfactual. The counterfactual function loaded must be named Counterfactual and it must be defined as Counterfactual <- function(X){function goes here} . Example: Counterfactual <- function(X){2*X} "))
                        }else{
                          if(is.function(Counterfactual) == FALSE){
                ...
                if(is.null(input$weight)){
                  ...
                  if(file_ext(input$weight$name)!="csv"){
                    ...
                    if(length(Weight)!=length(X)){
                      return(ggplot()+ggtitle(sprintf("Warning:The amount of exposure values and weight values are different,\n there are %d exposure values, %d weight values." , length(X), length(Weight))))
                    }else{
            ...
            if(is.null(input$RRfun)){
              return(ggplot()+ggtitle("Please load Relative Risk function in an R file with the function specified as: \n RRfunction <- function(X, theta){function goes here}. Example: RRfunction <- function(X, theta){exp(theta*X)} "))
            }else{
              if(file_ext(input$RRfun$name)!="R"){
                return(ggplot()+ggtitle("Warning: Incorrect file extension. Relative Risk function must be written in an R file"))
              }else{
                source(input$RRfun$datapath)
                if(exists("RRfunction") == FALSE){
                  return(ggplot()+ggtitle("Warning: There is no function named RRfunction. The Relative Risk function loaded must be named RRfunction and it must be defined as RRfunction <- function(X, theta){function goes here}. Example: RRfunction <- function(X, theta){exp(theta*X)}  "))
                }else{
                  if(is.function(RRfunction) == FALSE){
                    ...
                    if(is.null(input$thetahat)){
                      ...
                      if(file_ext(input$thetahat$name)!="csv"){
                        ...
                        if(is.numeric(try(RRfunction(0,theta))) == FALSE || is.na(RRfunction(0,theta))){
                          ...
                          if(RRfunction(0,theta) != 1){
                            return(ggplot()+ggtitle("Warning: Relative Risk function at minimum exposure level is not equal to one."))
                          }else{
                            RR <- RRfunction
                            if (input$w==0) {
                              ...
                              if(input$PIForPAF > 1){
                                if(input$Cft == 1){
                                  ...
                                  if(is.null(input$Cftfun)){
                                    ...
                                    if(file_ext(input$Cftfun$name)!="R"){
                                      rreturn(ggplot()+ggtitle("Warning: Incorrect file extension. Counterfactual function must be written in an R file"))
                                    }else{
                                      source(input$Cftfun$datapath)
                                      if(exists("Counterfactual") == FALSE){
                                        return(ggplot()+ggtitle("Warning: There is no function named Counterfactual. The counterfactual function loaded must be named Counterfactual and it must be defined as Counterfactual <- function(X){function goes here} . Example: Counterfactual <- function(X){2*X} "))
          ...
          if(xmin<0){
            ...
            if(rr < 5){
              ...
              RR <- switch(rr, function(X,theta){exp(theta[2]*X+theta[3])+theta[1]},
                           ...
                           function(X,theta){theta[4]*X^3+ theta[3]*X^2+theta[2]*X+theta[1]})
              if(RR(xmin,theta)!=1){
                return(ggplot()+ggtitle(" Relative Risk function at minimum exposure level must be equal to one."))
              }else{
                if (input$w==0) {
                              ...
                              if (input$w==0) {
                                ...
                                if(is.null(input$weight)){
                                  ...
                                  if(file_ext(input$weight$name)!="csv"){
                                    ...
                                    if(length(Weight)!=length(X)){
                                      return(ggplot()+ggtitle(sprintf("Warning:The amount of exposure values and weight values are different,\n there %d exposure values, %d weight values." , length(X), length(Weight))))
                                    }else{
                                      PAF.Plot <- sensitivity.pif(X=X, thetahat = theta, rr = RR, cft = function(X){xmin}, weights = Weight, title = "Sensitivity Analysis for Population Attributable Fraction \n (PAF)")
                                      if(input$PIForPAF==1){
                                        #Checar qué onda con multiconvex
