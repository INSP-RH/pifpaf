...
#' 
#' @description  Estimates a 1 - alpha pivotal confidence interval for the potential impact
#' fraction \code{pif} using a boostrap approximation. 
#' 
#' @param X         Random sample (vector or matrix) which includes exposure and
#'   covariates.
#'   
#' @param thetahat  Maximum Likelihood estimator (vector or matrix) of \code{theta} for the 
...
pif.confidence.bootstrap <- function(X, thetahat, thetavar, rr,         
  ...
  for (i in 1:.nboost){
    ...
    .whichX  <- sample(.index, nrow(.X), replace = TRUE, prob = weights) #Resampling according to probability weights
    .Xboost  <- .X[.whichX, ]                                            #Rows = individuals; columns = variables
