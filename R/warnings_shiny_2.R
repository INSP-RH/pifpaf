#' @title Warnings plot
#' 
#' @description This function has the possible warnings and output messages in  PIFApp for plots.
#' 
#' @param Error Number of corresponding error
#' 
#' @export
...
Warnings2 <- function(Error){
  switch (Error,
          ...
          return(ggplot()+ggtitle(TeX('Minimum $a$  must be smaller than the maximum $a$, and minimum $b$ must be smaller than the maximum $b$ '))),
          return(ggplot()+ggtitle("The Counterfactual 3D graph is not defined when theoretical minimum exposure value is different to 0"))
