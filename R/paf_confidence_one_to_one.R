...
#' thetalow <- 0.1
#' thetaup  <- 0.7
#' paf.confidence.one2one(X, thetahat, function(X, theta){exp(theta*X)}, thetalow, thetaup)
#' 
#' #Approximate method:
#' Xmean <- mean(X)
#' Xvar  <- var(X)
#' paf.confidence.one2one(Xmean, thetahat, function(X, theta){exp(theta*X)}, thetalow, thetaup, 
