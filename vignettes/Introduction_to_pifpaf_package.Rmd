...
3. A Fisher consistent or asymptotically consistent, asymptotically normal estimator $\hat{\theta}$ of $\theta$. 
...
\end{equation}
If  $\hat{\theta}$ is Fisher consistent, these are consistent estimators of the PIF and the PAF respectively. Furthermore if the Relative Risk $RR$ is either convex, concave or Lipschitz continuous as a function of $\theta$ and $\hat{\theta}$ is asymptotically consistent, the estimators have asymptotic consistency. 

### Kernel Based Potential Impact Fraction

Define the Relative Risk $RR:\mathcal{X} \times \Theta \to I \subset (0,\infty)$ (the additional hypotheses used for the [empirical method](#the-empirical-potential-impact-fraction) are not necessary).  Let $\hat{f}$ denote a kernel density obtained from the random sample of $X\in\mathcal{X}\subseteq\mathbb{R}^p$. Let $\hat{\theta} \in \Theta \subset \mathbb{R}^q$ be an estimator of  $\theta$. We define the functions:
...
\end{equation}
is a Fisher consistent estimator of the Potential Impact Fraction ($\textrm{PIF}$) if $\hat{\theta}$ is Fisher consistent. Furthermore if the Relative Risk $RR$ is either convex, concave or Lipschitz continuous as a function of $\theta$ and $\hat{\theta}$ is asymptotically consistent, the estimators have asymptotic consistency. 
...
The approximate method solely requires the sample mean $m$ and variance $s^2$, not the whole sample. If a random sample is available, the other methods should be preferred.  
...
```{r fig.width=7, fig.height=4}
pif.plot(X = X, thetalow = thetalow, thetaup = thetaup, rr = rr, cft = cft, method = "kernel", n = 100, adjust = 2, ktype = "triangular", confidence_method = "bootstrap", confidence = 99)
```
