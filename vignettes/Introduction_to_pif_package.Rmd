...
\begin{equation}
\widehat{\textrm{PIF}}= 1-\frac{RR(\textrm{cft}(m),\theta) + \frac{1}{2}\sum\limits_{i=1}^n\sum\limits_{j=1}^n \textrm{Cov}(X_i,X_j)\frac{\partial^2 RR\big(\textrm{cft}(X),\theta\big)}{\partial X_i \partial X_j}\Big|_m}{RR(m;\hat{\theta})+\frac{1}{2} \sum\limits_{i=1}^n\sum\limits_{j=1}^n \textrm{Cov}(X_i,X_j)\frac{\partial^2 RR\big(X,\theta\big)}{\partial X_i \partial X_j}\Big|_m}.
\end{equation}
...
The bootstrap method is the recommended method to calculate confidence intervals for the kernel and empirical point estimate. However this method can not be used while using the approximate point estimate, for only mean and variance are available. Therefore other methods such as the linear and log-linear methods were developed to calculate the confidence interval of the PIF, additionally the inverse and one to one methods can be used for some cases of the PAF.  All, but the one to one method, consider $\hat{\theta}$ to be a consistent estimator of $\theta$ such that it is asymptotically normal, i.e. $\hat{\theta} \dot{\sim} \mathcal{N}(\theta, \sigma_{\theta}^2)$. The following table shows the methods to estimate confidence intervals and when each of the methods can be used.

\begin{table}[H]
    \centering
    \begin{tabular}{cccc}
        Confidence Interval & Point Estimate & PAF or PIF & Extra Assumptions \\ \hline
        \hline
    \multirow{2}{*}{Bootstrap }
 & Kernel &\multirow{2}{*}{PIF \& PAF} &\multirow{2}{*}{None}\\
 & Empirical &  &\\ 
      \hdashline  
    \multirow{2}{*}{Linear}
 & Empirical & \multirow{2}{*}{PIF \& PAF}&\multirow{2}{*}{None}\\ 
 & Approximate && \\
    \hdashline
     \multirow{2}{*}{Log-linear}
 & Empirical & \multirow{2}{*}{PIF \& PAF}&\multirow{2}{*}{None}\\ 
 & Approximate && \\
 \hdashline
     \multirow{2}{*}{Inverse}
 & Empirical & \multirow{2}{*}{PAF} & \multirow{2}{*}{$RR(X,\theta)$ is injective in $X$}\\
 & Approximate & &\\
 \hdashline
     \multirow{2}{*}{One to one}
 & Empirical & \multirow{2}{*}{PAF} & \multirow{2}{*}{$RR(X,\theta)$ is injective in $X$ and $\theta$}\\ 
 & Approximate & & \\
 \hline 
 \hline
    \end{tabular}
    \caption{Usage of confidence interval methods}
    \label{fig:CI_methods}
\end{table}
Remember that to calculate the point estimate  of $\widehat{\textrm{PIF}}$ the relative risk function $RR(X,\theta)$ and the counter-factual function $\textrm{cft}(X)$ must be continuously differentiable in terms of $X$.
A brief explanation of each of the methods is given below.

#### Bootstrap
Bootstrap consists on resampling with replacement several times from a given random sample, in this case from the random sample of exposure values $X_1,X_2,\cdots X_n$.  For each sample $X^{j}=X_1^{j}, X_2^j, \cdots, X_n^{j}$ a value $\theta_j$ is simulated from a normal distribution with mean $\hat{\theta}$ and variance $\sigma^2_{\theta}$. For each $X^j$ and $\theta_j$, $\widehat{\textrm{PIF}}_j$ is calculated with the selected method (empirical or kernel). Once  $\widehat{\textrm{PIF}}_j$ has been calculated for every sample $X^j$ a confidence interval is calculated using the pivotal method  @wasserman2006nonparametric.

#### Linear
For the linear, log-linear and inverse method the variance of $\widehat{\textrm{PIF}}$ or a transformation of  $\widehat{\textrm{PIF}}$ is calculated for a fixed value of $\theta$. Then simulations of $\theta$ and conditional variance is used to account uncertainty in $\theta$.
The linear method considers Taylor's first order approximation (linearization) of $\widehat{\textrm{PIF}}$ and the variance for $\widehat{\textrm{PIF}}$ is calculated as the variance of the linearization. This approach is better known as the Delta Method @casella2002statistical. 

#### Log-linear
First we calculate the $(1-\alpha)$ confidence interval for $\textrm{log}(1-\widehat{\textrm{PIF}})$ ($\textrm{IC}_{\textrm{log}}$) , then it is transformed as $1-e^{\textrm{IC}_{\textrm{log}}}$. Since $1-e^x$ is a one to one function the confidence interval of $\widehat{\textrm{PIF}}$ is at least a  $(1-\alpha)$ confidence interval @bar1999confidence.

#### Inverse
The inverse method can be used only for confidence intervals of $\widehat{\textrm{PAF}}$. The confidence interval ($\textrm{IC}_{RR}$) for $RR(X,\theta)$ (approximate method) or for $\sum_{i=1}^{\infty}w_iRR(X_i,\theta)$ (empirical method) is calculated and then transformed to a confidence interval of $\widehat{\textrm{PAF}}$ by $1-1/\textrm{IC}_{RR}$. Once again the transformation $1-1/x$ is injective, therefore from a $(1-\alpha)$ confidence interval for $\textrm{IC}_{RR}$ a confidence interval of $\widehat{\textrm{PAF}}$  at least  $(1-\alpha)$ confidence can be calculated @bar1999confidence.

#### One to one
The one to one method is similar to the inverse method, for the confidence interval $\textrm{IC}_{RR}$ is calculated. The difference lies on how uncertainty on $\theta$ is calculated. The inverse method uses simulations of $\theta$, while the one to one method considers the upper and lower bounds of a $1-\beta$ confidence interval of $\theta$ to calculate a $1-\alpha$ confidence interval $\textrm{IC}_{RR}$, where $\alpha>\beta$ @bar1999confidence.
...
When calculating confidence intervals for the Population Attributable Fraction additional methods are available: `"one2one"` and `"inverse"`. These are "smaller" confidence intervals, however additional assumptions are required. The `"inverse"` method requires that the Relative Risk is injective as a function of $X$. The `"one2one"` requires that the Relative Risk is injective as a function of $X$ and $\theta$. The `"one2one"` instead of receiving the estimated variance of $\theta$ it receives $\theta$'s confidence interval $[\theta_{\textrm{low}}, \theta_{\textrm{high}}]$]. 
