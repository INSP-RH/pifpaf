...
The bootstrap method is the recommended method to calculate confidence intervals while using the kernel and empirical point estimate. However this method cannot be used while using the approximate point estimate, for only mean and variance are available. Therefore other methods such as the linear and log-linear methods were developed to calculate the confidence interval of the PIF, additionally the inverse and one to one methods can be used for some cases of the PAF.  All, but the one to one method, consider $\hat{\theta}$ to be a consistent estimator of $\theta$ such that it is asymptotically normal, i.e. $\hat{\theta} \dot{\sim} \mathcal{N}(\theta, \sigma_{\theta}^2)$ where $\hat{\sigma}_{\theta}^2$ is an estimator of $\sigma_{\theta}^2$. The confidence might change with the estimation method, however confidence intervals are calculated for the real value of the $\textrm{PIF}$. This is if there was a large number of random samples and a $(1-\alpha)$ confidence interval was calculated for each sample, then the real value of $\textrm{PIF}$ will be in $(1-\alpha)\times 100\%$ of the confidence intervals. This would happen with every estimation method. The following table shows the methods to estimate confidence intervals and when each of the methods can be used. 
...
|Log-linear |Empirical & Appproximate| PIF & PAF  |None|
|Inverse    |Empirical & Appproximate| PAF        |$RR(X,\theta)$ is injective in $X$|
|One to one |Empirical & Appproximate| PAF        |$RR(X,\theta)$ is injective in $X$ and $\theta$|
