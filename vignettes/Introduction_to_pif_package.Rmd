...
```{r fig.width=7, fig.height=4}
pif.sensitivity(X = X,thetahat = thetahat, rr = rr, title = "Sensitivity Analysis for Population Attributable Fraction", n = 20, m = 50)
```
...
minb <- -3
maxb <- -1
pif.counterfactual.heatmap(X=X, thetahat = thetahat, rr = rr, mina = mina, maxa = maxa, minb = minb, nmesh = 5)
```
...
pif.counterfactual.heatmap(X=X, thetahat = thetahat, rr = rr, mina = mina, maxa = maxa, minb = minb, cft = cft, nmesh = 5)
```
...
```{r fig.width=7, fig.height=4}
pif.sensitivity(X, thetahat = thetahat, rr = RR, n = 20, m = 50)
```
